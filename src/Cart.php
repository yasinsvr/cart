<?php namespace YasinSVR\Cart;

use YasinSVR\Cart\Exceptions\InvalidConditionException;
use YasinSVR\Cart\Exceptions\InvalidItemException;
use YasinSVR\Cart\Helpers\Helpers;
use YasinSVR\Cart\Validators\CartItemValidator;
use YasinSVR\Cart\Exceptions\UnknownModelException;

/**
 * Class Cart
 * @package YasinSVR\Cart
 */
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = "\\YasinSVR\\Cart\\CartCondition";

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of YasinSVR\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition) throw new InvalidConditionException('Argument 1 must be an instance of \'YasinSVR\Cart\CartCondition\'');
